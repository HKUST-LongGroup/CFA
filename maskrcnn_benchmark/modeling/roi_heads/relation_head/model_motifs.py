...
class FrequencyBias(nn.Module):
    ...
    def __init__(self, cfg, statistics, eps=1e-3):
        ...
        # sub_pred_dist = statistics['sub_pred_dist'].float()
        # obj_pred_dist = statistics['obj_pred_dist'].float()
        ...
        # sub_pred_dist = sub_pred_dist.view(-1, self.num_rels)
        # obj_pred_dist = obj_pred_dist.view(-1, self.num_rels)
        
        self.obj_baseline = nn.Embedding(self.num_objs*self.num_objs, self.num_rels)
        # self.sub_pred_baseline = nn.Embedding(self.num_objs, self.num_rels)
        # self.obj_pred_baseline = nn.Embedding(self.num_objs, self.num_rels)
        with torch.no_grad():
            self.obj_baseline.weight.copy_(pred_dist, non_blocking=True)
            # self.sub_pred_baseline.weight.copy_(sub_pred_dist, non_blocking=True)
            # self.obj_pred_baseline.weight.copy_(obj_pred_dist, non_blocking=True)
    ...
    def sub_pred_index_with_labels(self, labels):
        """
        :param labels: [batch_size, 2] 
        :return: 
        """
        return self.sub_pred_baseline(labels[:, 0])

    def sub_pred_index_with_probability(self, pair_prob):
        """
        :param labels: [batch_size, num_obj, 2] 
        :return: 
        """
        ...
        joint_prob = pair_prob[:,:,0].contiguous().view(batch_size, num_obj, 1)

        return joint_prob.view(batch_size, num_obj)  @ self.sub_pred_baseline.weight

    def obj_pred_index_with_labels(self, labels):
        """
        :param labels: [batch_size, 2] 
        :return: 
        """
        return self.obj_pred_baseline(labels[:, 1])

    def obj_pred_index_with_probability(self, pair_prob):
        """
        :param labels: [batch_size, num_obj, 2] 
        :return: 
        """
        ...
        joint_prob = pair_prob[:,:,1].contiguous().view(batch_size, num_obj, 1)

        return joint_prob.view(batch_size, num_obj*num_obj)  @ self.obj_pred_baseline.weight
