...
class PostProcessor(nn.Module):
    ...
    def forward(self, x, rel_pair_idxs, boxes):
        ...
        for i, (rel_logit, obj_logit, rel_pair_idx, box) in enumerate(zip(
            ...
            # print('rel_score', rel_scores, 'obj_score0', obj_scores0, 'obj_score1', obj_scores1)
...
class PKOPostProcessor(nn.Module):
    """
    From a set of classification scores, box regression and proposals,
    computes the post-processed boxes, and applies NMS to obtain the
    final results
    ...
    def __init__(
        ...
        """
        super(PKOPostProcessor, self).__init__()
        self.attribute_on = attribute_on
        self.use_gt_box = use_gt_box
        self.later_nms_pred_thres = later_nms_pred_thres
    ...
    def forward(self, x, rel_pair_idxs, boxes):
        ...
        # obj_pred_dist = statistics['obj_pred_dist'].float()
            pred_dist = self.statistic['pred_dist'].float()
            self.num_objs = pred_dist.size(0)
            obj_scores0 = obj_scores[rel_pair_idx[:, 0]]
            obj_scores1 = obj_scores[rel_pair_idx[:, 1]]
            subs = rel_pair_idx[:, 0]
            objs = rel_pair_idx[:, 1]
            sub_kg = self.statistic['sub_matrix'][obj_class[subs]]
            obj_kg = self.statistic['obj_matrix'][obj_class[objs]]
            pair_kg = self.statistic['pair_matrix'][obj_class[subs] * self.num_objs + obj_class[objs]]
            rel_logit = rel_logit + sub_kg.to(rel_logit[0].device) + obj_kg.to(rel_logit[0].device)
            # rel_logit = rel_logit + pair_kg.to(rel_logit[0].device)
            # rel_logit = rel_logit + obj_kg.to(rel_logit[0].device) + pair_kg.to(rel_logit[0].device)
            rel_class_prob = F.softmax(rel_logit, -1)
            rel_scores, rel_class = rel_class_prob[:, 1:].max(dim=1)
            rel_class = rel_class + 1
            # TODO Kaihua: how about using weighted some here?  e.g. rel*1 + obj *0.8 + obj*0.8
...
def make_roi_relation_pko_post_processor(cfg):
    attribute_on = cfg.MODEL.ATTRIBUTE_ON
    use_gt_box = cfg.MODEL.ROI_RELATION_HEAD.USE_GT_BOX
    later_nms_pred_thres = cfg.TEST.RELATION.LATER_NMS_PREDICTION_THRES
    statistic = get_dataset_statistics(cfg)

    postprocessor = PKOPostProcessor(
        attribute_on,
        use_gt_box,
        later_nms_pred_thres,
        statistic,
