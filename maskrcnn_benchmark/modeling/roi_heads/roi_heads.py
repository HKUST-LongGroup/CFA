    ...
    def forward(self, features, proposals, targets=None, logger=None, cur_iter=None):
        ...
        if self.cfg.MODEL.RELATION_ON:
            ...
            if self.cfg.TYPE == 'extract_aug':
                tail_dict = self.relation(features, detections, targets, logger)
                return tail_dict
            if self.cfg.TYPE == 'extract_cfa_feat':
                origital_feat_dict, cfa_feat_dict = self.relation(features, detections, targets, logger)
                return origital_feat_dict, cfa_feat_dict
            x, detections, loss_relation = self.relation(features, detections, targets, logger, cur_iter=cur_iter)
