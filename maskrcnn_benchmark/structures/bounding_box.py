...
class BoxList(object):
    ...
    def append(self, box_list):
        device = self.bbox.device
        self.bbox = torch.cat((self.bbox, box_list.bbox.to(device)), dim=0)
        for k, _ in box_list.extra_fields.items():
            self.extra_fields[k] = torch.cat((self.extra_fields[k], box_list.get_field(k).to(device)), dim=0)
